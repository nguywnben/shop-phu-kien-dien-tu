...
// Xử lý dữ liệu hiển thị: ưu tiên dữ liệu cũ (old input) nếu có lỗi, ngược lại dùng dữ liệu từ DB ($brand)
$brand_id = $brand['id'] ?? null;
$name_value = $name_old ?: ($brand['name'] ?? '');
$slug_value = $slug_old ?: ($brand['slug'] ?? '');
$logo_value = $brand['logo'] ?? ''; // Logo hiện tại (đường dẫn lưu trong DB)
$status_value = $status_old ?: ($brand['status'] ?? '');

?>
<main>
    <div class="mx-auto max-w-4xl p-4 md:p-6"> <div x-data="{ pageName: 'Sửa Thương hiệu: <?= htmlspecialchars($brand['name'] ?? 'Không tìm thấy') ?>'}">
        ...
        <form id="edit-brand" method="POST"
            action="?page=brands&action=update&id=<?= htmlspecialchars($brand_id) ?>"
            enctype="multipart/form-data">

            <input type="hidden" name="id" value="<?= htmlspecialchars($brand_id) ?>">
                    ...
                    <div class="p-5 space-y-6 sm:p-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="slug" id="brandSlug"
                                class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800"
                                placeholder="Ví dụ: ten-thuong-hieu"
                                value="<?= htmlspecialchars($slug_value) ?>" />
                            <?php if ($slug_error): ?>
                        ...
                        <div>
                            <label for="status"
                                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Trạng
                        ...
                        <div>
                            ...
                            <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">Logo hiện tại:</p>
                            <img src="../../../../Assets/client/img/<?= htmlspecialchars($logo_value) ?>" style="max-width: 100px; max-height: 100px;" 
                                 alt="Logo hiện tại"
                                 class="mb-2 rounded-lg border border-gray-200 dark:border-gray-700" />
                            <input type="hidden" name="old_logo" value="<?= htmlspecialchars($logo_value) ?>">
                            <?php endif; ?>
