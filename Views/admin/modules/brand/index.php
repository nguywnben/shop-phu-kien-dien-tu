                                    ...
                                    <?php foreach ($brands as $brand): ?>
                                        <tr>
                                            ...
                                            <td class="px-5 py-4 sm:px-6">
                                                <div class="flex items-center">
                                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                                        <img src="../../../../Assets/client/img/<?= $brand['logo'] ?>"
                                            ...
                                            <td>
                                                <a href="admin.php?page=brands&action=edit&id=<?= $brand['id'] ?>"
                                                ...
                                                <form method="post" action="admin.php?page=brands&action=delete"
        ...
        if (isset($pagination) && $pagination['totalPages'] > 1): ?>
            <div class="w-full flex flex-col items-center justify-center mt-6 mb-4">
                <div class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                    Hiển thị
                    <span
                        class="font-semibold text-brand-600 dark:text-brand-400"><?= $pagination['totalBrands'] - ($pagination['currentPage'] - 1) * $pagination['limit'] ?></span>
                    đến
                    <span
                        class="font-semibold text-brand-600 dark:text-brand-400"><?= max($pagination['totalBrands'] - $pagination['currentPage'] * $pagination['limit'] + 1, 1) ?></span>
                    trên tổng số
                    <span class="font-semibold text-brand-600 dark:text-brand-400"><?= $pagination['totalBrands'] ?></span>
                    thương hiệu.
                </div>
                <nav class="flex flex-wrap gap-1 justify-center items-center" aria-label="Pagination">
                    <!-- Previous -->
                    <a href="?page=brands&action=index&page_num=<?= $pagination['currentPage'] - 1 ?>"
                    ...
                    <?php for ($i = 1; $i <= $pagination['totalPages']; $i++): ?>
                        <a href="?page=brands&action=index&page_num=<?= $i ?>" class="px-3 py-1 rounded-full border text-sm font-medium transition
