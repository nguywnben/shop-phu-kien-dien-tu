<section class="products container section--lg">
    <p class="total__products">Chúng tôi tìm thấy
        <span><?php echo isset($totalProducts) ? (int) $totalProducts : 0; ?></span> sản phẩm cho bạn!</p>
        ...
        <?php foreach ($products as $product): ?>
            ...
            $resolveCandidate = function ($val) use ($serverRoot) {
                ...
                if (strpos($val, '/') !== false || strpos($val, '\\') !== false) {
                    ...
                    if (file_exists($serverRoot . DIRECTORY_SEPARATOR . $alt))
                        return $alt;
                    return $val;
                }
                $cand = 'Assets/client/img/' . $val;
                if (file_exists($serverRoot . DIRECTORY_SEPARATOR . $cand))
                    return $cand;

                $cand2 = 'uploads/products/' . $val;
                if (file_exists($serverRoot . DIRECTORY_SEPARATOR . $cand2))
                    return $cand2;
                return $val;
            };
            ...
            <div class="product__item">
                <div class="product__banner">
                    <a href="index.php?page=details&product_id=<?php echo $product['id']; ?>" class="product__images">
                        <img src="<?php echo htmlspecialchars($defaultImg); ?>"
                            alt="<?php echo htmlspecialchars($product['name']); ?>" class="product__img default" />
                        <img src="<?php echo htmlspecialchars($hoverImg); ?>"
                            alt="<?php echo htmlspecialchars($product['name']); ?>" class="product__img hover" />
                    </a>
                    <div class="product__actions">
                        <form method="POST" action="index.php?page=wishlist&action=add" style="display: inline;">
                            <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>" />
                            <button type="submit" class="action__btn" aria-label="Thêm vào Danh sách yêu thích"
                                onclick="if(!<?php echo isset($_SESSION['login']) ? 'true' : 'false'; ?>) {event.preventDefault(); window.location.href='index.php?page=login&action=index'; return false;}">
                ...
                <div class="product__content">
                    <span
                        class="product__category"><?php echo htmlspecialchars($product['category_name'] ?? 'Khác'); ?></span>
                    <a href="details.html">
    ...
    <?php if (isset($totalPages) && $totalPages > 1): ?>
        <ul class="pagination">
            ...
            if ($start > 1) {
                ...
                if ($start > 2)
                    echo '<li><span class="pagination__dots">...</span></li>';
            }
            for ($i = $start; $i <= $end; $i++) {
                $active = $i === $cur ? ' active' : '';
